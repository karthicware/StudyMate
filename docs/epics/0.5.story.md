...
## Tasks / Subtasks
- [ ] Task 1: Install Playwright dependencies (AC: 1)
  - [ ] Run `npm init playwright@latest` to initialize Playwright
  - [ ] Select TypeScript as test language
  - [ ] Configure test directory as `e2e/`
  - [ ] Install browsers when prompted (Chromium, Firefox, WebKit)
  - [ ] Verify Playwright dependencies added to package.json devDependencies
- [ ] Task 2: Configure Playwright for Angular (AC: 1)
  - [ ] Update `playwright.config.ts` with Angular dev server settings
  - [ ] Set baseURL to `http://localhost:4200`
  - [ ] Configure webServer to start Angular dev server automatically
  - [ ] Set timeout values appropriate for Angular app startup
  - [ ] Configure test output directory and reporters
- [ ] Task 3: Verify Playwright MCP server availability (AC: 2)
  - [ ] Check that Playwright MCP server is configured in Claude Code
  - [ ] Verify MCP connection using `mcp__playwright__browser_navigate` tool
  - [ ] Document MCP server configuration requirements
  - [ ] Test basic MCP commands (navigate, snapshot, click)
- [ ] Task 4: Create sample E2E test using Playwright (AC: 3)
  - [ ] Create `e2e/app.spec.ts` test file
  - [ ] Write test: "should display app title"
  - [ ] Use Playwright test fixtures and assertions
  - [ ] Follow Playwright best practices (role-based locators)
  - [ ] Add descriptive test names
- [ ] Task 5: Integrate Playwright with Angular routing (AC: 3)
  - [ ] Test navigation to homepage
  - [ ] Verify Angular app loads correctly
  - [ ] Test that default route renders
  - [ ] Use web-first assertions (`toBeVisible`, `toHaveText`)
- [ ] Task 6: Configure Playwright test scripts (AC: 4)
  - [ ] Add `"e2e": "playwright test"` script to package.json
  - [ ] Add `"e2e:ui": "playwright test --ui"` for UI mode
  - [ ] Add `"e2e:headed": "playwright test --headed"` for visible browser
  - [ ] Add `"e2e:debug": "playwright test --debug"` for debugging
  - [ ] Test all scripts run successfully
- [ ] Task 7: Create comprehensive E2E test for MCP validation (AC: 5)
  - [ ] Create `e2e/mcp-integration.spec.ts` test file
  - [ ] Test browser navigation using MCP tools
  - [ ] Test element interaction (click, type) using MCP
  - [ ] Test snapshot and screenshot capabilities
  - [ ] Verify assertions work correctly
  - [ ] Test multiple pages/routes if available
- [ ] Task 8: Configure Playwright for CI/CD readiness (AC: 1)
  - [ ] Configure browsers to run in headless mode by default
  - [ ] Set up test retries for flaky test handling
  - [ ] Configure parallel test execution
  - [ ] Set up HTML reporter for test results
  - [ ] Configure screenshot/video capture on failure
- [ ] Task 9: Verify zero console errors (AC: 6)
  - [ ] Add test to capture and assert browser console logs
  - [ ] Verify no errors or warnings during test execution
  - [ ] Use Playwright's console event listener
  - [ ] Fail tests if console errors detected
- [ ] Task 10: Document Playwright MCP setup (AC: 2)
  - [ ] Document Playwright MCP integration in README.md
  - [ ] Add instructions for running E2E tests
  - [ ] Document MCP server requirements
  - [ ] Create troubleshooting guide for common issues
  - [ ] Document best practices from playwright-rules.md
