...
- [ ] Add `getHallAmenities(hallId: string)` method → `GET /owner/halls/{hallId}/amenities`
- [ ] Add `updateHallAmenities(hallId: string, amenities: string[])` method → `PUT /owner/halls/{hallId}/amenities`
...
- [ ] Add success indicator (checkmark ✓) that auto-hides after 2 seconds
...
- [ ] Write E2E Test 1: Owner can check AC amenity → verify API call → verify success indicator
- [ ] Write E2E Test 2: Owner can check Wi-Fi amenity → verify API call
- [ ] Write E2E Test 3: Owner can select both amenities → verify both in API payload
...
- **Test Data Requirements**:
  ...
  - Create 2 test study halls in `studymate_test` database:
    - Hall 1: amenities `["AC"]`
...
**Status**: ✅ **FULLY COMPLIANT** - All 6 anti-patterns avoided
